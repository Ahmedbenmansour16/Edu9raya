...
public class ViewCoursByModule {
    ...
    // New method to pre-select a module when navigated from ModuleList
    public void initializeWithModule(Module module) {
        moduleComboBox.getSelectionModel().select(module);
    }

    private void loadCoursesByModule(int moduleId) {
        vboxContainer.getChildren().clear();
        List<Cour> courses = courService.retrieveByModuleId(moduleId);
        if (courses.isEmpty()) {
            Label noCoursesLabel = new Label("No courses found for this module.");
    ...
    private Pane createCourseCard(Cour course) {
        ...
        // Module Name
        Label moduleNameLabel = new Label("Module: " + (course.getModule() != null && course.getModule().getNom() != null ? course.getModule().getNom() : "N/A"));
        ...
        // Module Teacher
        Label moduleTeacherLabel = new Label("Teacher: " + (course.getModule() != null && course.getModule().getEnseignant() != null ? course.getModule().getEnseignant() : "N/A"));
        ...
        card.getChildren().addAll(titleLabel, codeLabel, levelLabel, categoryLabel, descriptionLabel, moduleNameLabel, moduleTeacherLabel, updateButton, deleteButton);
        return card;
    }
        ...
        confirmationAlert.showAndWait().ifPresent(response -> {
            if (response == ButtonType.OK) {
                try {
                    ...
                    // Refresh the course list for the current module
                    Module selectedModule = moduleComboBox.getValue();
                    if (selectedModule != null) {
                        loadCoursesByModule(selectedModule.getId());
                    }
                } catch (Exception e) {
                    Alert errorAlert = new Alert(Alert.AlertType.ERROR);
                    errorAlert.setTitle("Error");
