...
class Evenement
{
    ...
    #[Assert\Length(min: 10, maxMessage: "La description doit contenir au moins {{ limit }} caractères")]
    ...
    private ?int $nbrParticipantsMax = null;

    #[ORM\Column(length: 255)]
    #[Assert\NotBlank(message: "Le status ne peut pas être vide")]
    #[Assert\Choice(choices: ['Planifié', 'En cours', 'Terminé'], message: "Le status doit être l'une de ces valeurs : Planifié, En cours, Terminé")]
    private ?string $status = null;

    #[ORM\OneToMany(targetEntity: Feedback::class, mappedBy: 'idEvent')]
    ...
    {
        return $this->nbrParticipantsMax;
    }

    public function setNbrParticipantsMax(int $nbrParticipantsMax): static
    {
        $this->nbrParticipantsMax = $nbrParticipantsMax;
        return $this;
    }
    ...
    public function removeFeedback(Feedback $feedback): static
    {
        if ($this->feedback->removeElement($feedback)) {
            ...
            if ($feedback->getIdEvent() === $this) {
                $feedback->setIdEvent(null);
            }
        }
